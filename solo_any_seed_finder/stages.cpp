...
constexpr BoxI32 OIL_BOX(-250, 50, 0, 300);
...
constexpr std::array<std::tuple<ResourceType, BoxI32, float>, 6> MALL_BOXES{
    std::make_tuple(COPPER, BoxI32(-45, -40,   0, -32), 1.f), // main copper
...
Finder<SeedCache>::EvalResult stage1_eval(
    ...
    BoxI32 rotated = seed_cache->mixer_copper_bounding_box.rotated_counter_clockwise(seed_cache->mixer_direction);
    PositionI32 center = rotated.center();
    center.x = rotated.side_position(WEST);
    BoxI32 oil_box = (OIL_BOX + center).rotated(seed_cache->mixer_direction);

    bool any_oil = false;
    for (const auto& p : patches[OIL]) {
        if (oil_box.contains(p.pos)) {
            any_oil = true;
            break;
        }
    }

    return { !any_oil, best_score };
}
...
Finder<SeedCache>::EvalResult stage2_eval(
    ...
    for (int32_t x = MIN_EDGE_MALL_MIXER_PATCHES_DISTANCE; x < MAX_EDGE_MALL_MIXER_PATCHES_DISTANCE; x += MALL_OFFSET_SAMPLING_DISTANCE) {
        for (int32_t y = -MAX_MALL_MIXER_PATCHES_SHIFT; y < MAX_MALL_MIXER_PATCHES_SHIFT; y += MALL_OFFSET_SAMPLING_DISTANCE) {
            ...
            for (const auto& t : MALL_BOXES) {
                BoxI32 box = (std::get<BoxI32>(t) + mall_boxes_offset).rotated(direction);
                float good_area = area_covered_by_patch(patches[std::get<ResourceType>(t)], box, MALL_AREA_COMPUTE_SAMPLING_DISTANCE1);
                if (good_area <= 0.f) {
                    score = -INFINITY;
                    break;
                }
                ...
                for (ResourceType resource = IRON; resource < NB_RESOURCE_TYPE; resource++) {
                    if (resource == std::get<ResourceType>(t)) continue;
                    good_area -= 0.6f * (float)area_covered_by_patch(patches[resource], box, MALL_AREA_COMPUTE_SAMPLING_DISTANCE2);
                }
                if (good_area <= 0.f) {
                    score = -INFINITY;
                    break;
                ...
                score += good_area * std::get<float>(t);
            }

            score /= MALL_PATCHES_WEIGHT * boxes_factors_sum;

            BoxI32 burner_box = (std::get<BoxI32>(MALL_BOXES[1]) + mall_boxes_offset).rotated(direction);
...
Finder<SeedCache>::EvalResult stage3_eval(
    ...
    BoxI32 rotated = seed_cache->mixer_copper_bounding_box.rotated_counter_clockwise(direction);
    PositionI32 center = rotated.center();
    center.x = rotated.side_position(WEST);
    BoxI32 oil_box = (OIL_BOX + center).rotated(direction);

    bool any_oil = false;
    for (const auto& p : r_patches[OIL]) {
        if (oil_box.contains(p.pos) && noise.elevation(settings, precompute, p.pos) > -10) {
            any_oil = true;
            break;
        }
    }
